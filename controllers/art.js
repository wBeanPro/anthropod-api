...
exports.GET_ALL_ART = (req, res, next) => {
    ...
    .exec((err, artCollection) => {
        if (err) {
            ...
            return res.status(200).send({isSuccess:true, artCollection:artCollection})
        }
    })
}
...
exports.CREATE_ART = async(req, res, next) => {
    ...
    try {
        ...
        await newArt.save();
        return res.status(200).send({isSuccess: true, message: 'file has successfully uploaded'})
    } catch (error) {
...
exports.UPDATE_ART = async(req, res, next) => {
    ...
    ArtModel.findByIdAndUpdate(
        ...
        (err, updatedItem) =>{
            if (err) return res.status(500).send({isSuccess:false, message: err.message});
            else if (updatedItem){
                return res.status(200).send({isSuccess:true, message:`${updatedItem.name} has succesfully updated!`})
            }
        }
...
exports.GET_ART_BY_ID = (req, res, next) => {
    ...
    .exec((err, doc) => {
        if (err) {
            ...
            return res.status(404).send({isSuccess:false, message: 'RESOURCE NOT FOUND!'})
        }
    })
}
...
exports.DELETE_ART = (req, res, next) => {
    ...
    ArtModel.findByIdAndDelete(
        {_id:id},
        ((err, doc) => {
            if (err) {
                return res.status(500).send({isSuccess: false, message: err.message})
            } else if (doc) {
                return res.status(200).send({isSuccess: true, message: `${doc.name} has been deleted!`})
            } else {
                return res.status(404).send({isSuccess:false, message: 'RESOURCE NOT FOUND!'})
            }
        })
